...
test_that("Test that the values of input parameters is valid", {
  expect_error(splitter(X=tibble(c(1)), target_index=1, split_size=0.2, seed=1), "ValueError: X must contains at least two observations and at least two columns")
...
test_that("Test that split result for each output is valid", {
  result <- splitter(X=TOY_X, target_index=1, split_size=0.25, seed=1)
  expected_Xtrain <- tibble(y=c(2,6,3), z=c(3,3,2))
  expected_ytrain <- tibble(x=c(1,5,4))
  expected_Xtest <- tibble(y=c(4), z=c(3))
  expected_ytest <- tibble(x=c(3))
